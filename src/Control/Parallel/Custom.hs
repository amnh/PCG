...
parZipWith3 strat f x y z
  | length x /= length y || length y /= length z = error "lengths did not match in parZipWith3"
  | otherwise = withStrategy (parTraversable strat) $ zap (zipWith f x y) z
