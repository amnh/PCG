                               ...
                               )
{
    ...
    /* Conceptually,
       gap_row = cm_get_precal_row (precalcMtx, gap_char, seq2_len);
    */
    gap_row = precalcMtx + (costMtx->gap_char * seq2_len);
    len--;
    while (i < end_row) {
        /** Invariants block */
        cur_seq1  = seq1->seq_begin[i];
...
static inline int
algn_fill_plane ( const seq_p longerCharuence
                , int *precalcMtx
                 // Leading gap included in input lengths
                ,       size_t longerCharuenceLength //larger, horizontal dimension
                ...
                )
{
    ...
    // printf("longerCharuenceLength: %d\n", longerCharuenceLength);
    ...
    if (LOCAL_DEBUG_COST_M) {
        ...
        debugCostMatrixBuffer = malloc(longerCharuenceLength * lesserSequenceLength * sizeof(int));
    }
    if (DEBUG_DIR_M) {
        printf ("A\t");
    }
    ...
    if (LOCAL_DEBUG_COST_M) {
        ...
        printf("  x |    * ");
        for (i = 1; i < longerCharuenceLength; i++) {
            printf("%4d ", longerCharuence->seq_begin[i]);
        }
        printf("\n");
        printf(" ---+-");

        for (i = 0; i < longerCharuenceLength; i++) {
            printf("-----");
        }
        printf("\n");
	...
	// Print cost matrix rows
	    ...
	    // Print cost matrix row header
            ...
            for (j = 0; j < longerCharuenceLength; j++) {
...
static inline int
algn_fill_plane_2 ( const seq_p longerChar
                  ,       int *precalcMtx
                  ,       int  longerChar_len
                  ,       int  len_lesserSeq
                  ,       int *curRow
                  ...
                  )
{
    ...
    if (width > len_lesserSeq) {
        width = len_lesserSeq; // width is at most len_lesserSeq
    }
    ...
    gap_row = cm_get_precal_row (precalcMtx, 0, len_lesserSeq); // We want the first horizontal row
     ...
     */
        ...
        // subset 3:
            ...
            algn_fill_first_row (curRow, dirMtx, width, gap_row);
            start_row    = 1;
            final_row    = (len_lesserSeq - width) + 1;
            start_column = 0;
            length       = width + 1;
    ...
    return next_prevRow[len_lesserSeq - 1];
}
/******************************************************************************/
...
algn_initialize_matrices_affine (int go,
    ...
    int /* lenShortSeq, */ longerChar_len, i = 1, j = 1, r;
    int *prev_extend_vertical;
    const int *gap_row;
...
//    lenShortSeq    = shortSeq->len - 1; //TODO: is this for deleting opening gap? This is currently unused
    ...
    longerChar_len            = longSeq->len  - 1; //TODO: is this for deleting opening gap?
...
algn_fill_plane_2d_affine_nobt (const seq_p shortSeq,
    ...
    assert (longerChar_len >= lenShortSeq);
    ...
    gap_row = cm_get_precal_row(precalcMtx, 0, longerChar_len);
    end_pos = (longerChar_len - lenShortSeq) + 8;
...
//         longSeqPrevElem,
    ...
    for (;i <= lenShortSeq; i++) {
        ...
        prev_extend_vertical       = init_extend_vertical +
                                       ((longerChar_len + 1) * ((i - 1) % 2));
        prev_extend_block_diagonal = init_extend_block_diagonal + ((longerChar_len + 1) * ((i - 1) % 2));
        prev_close_block_diagonal  = init_close_block_diagonal +
                                       ((longerChar_len + 1) * ((i - 1) % 2));
        extend_horizontal          = init_extend_horizontal + ((i % 2) * (longerChar_len + 1));
        extend_vertical            = init_extend_vertical + ((longerChar_len + 1) * (i % 2));
        extend_block_diagonal      = init_extend_block_diagonal + ((longerChar_len + 1) * (i % 2));
        close_block_diagonal       = init_close_block_diagonal + ((longerChar_len + 1) * (i % 2));
 ...
 */
static inline int
algn_nw_limit_2d ( const seq_p shorterChar
                 , const seq_p longerChar
                 , const cost_matrices_2d_p costMatrix
...
//    const SEQT *slongerChar, *sshorterChar;
    int *curRow,
        *precalcMtx;
...
int
algn_nw_2d ( const seq_p shorterChar
           , const seq_p longerChar
           , const cost_matrices_2d_p costMatrix
           , nw_matrices_p nw_mtxs
           , int deltawh )
{
    ...
    if(DEBUG_NW) {
        ...
        seq_print(longerChar);
        seq_print(shorterChar);
...
algn_print_dynmtrx_2d (const seq_p seq1, const seq_p seq2, nw_matrices_p alignment_matrices) {
  ...
  const seq_p longerChar  = seqLen1 > seqLen2 ? seq1 : seq2;
...
void
algn_backtrace_2d ( const seq_p shorterChar
                  , const seq_p longerChar
                  , seq_p ret_longerChar
                  , seq_p ret_shorterChar
...
void
algn_get_median_2d_with_gaps ( seq_p shorterChar
                             , seq_p longerChar
                             , cost_matrices_2d_p costMatrix
                             , seq_p medianToReturn
                             )
{
    SEQT *seq_begin_longerChar, *seq_begin_shorterChar;
    int interim;
    int i;
...
void
algn_get_median_2d_no_gaps (seq_p shorterChar, seq_p longerChar, cost_matrices_2d_p costMatrix, seq_p sm) {
    SEQT        *seq_begin_longerChar, *seq_begin_shorterChar;
...
void
algn_union (seq_p shorterChar, seq_p longerChar, seq_p unionToReturn)
{
    ...
    for (i = len - 1; i >= 0; i--) {
        seq_prepend (unionToReturn, (longerChar->seq_begin[i] | shorterChar->seq_begin[i]));
    }
}
