...
void algn_matx_print(alignment_matrices_t *alignMtx, size_t alphSize) {
    printf("\nMatrices:\n");
...
void
algn_mtx_clean_direction_matrix (alignment_matrices_t *alignMtx) {
 ...
 */
inline void
algn_mtx_setup_size (alignment_matrices_t *alignMtx, size_t len_char1, size_t len_char2, size_t len_char3, size_t alphabetSize) {
    if(DEBUG_MAT) {
        printf("\n---algn_mtx_setup_size\n");
    ...
    if (len_char3 == 0) {           /* If the size setup is only for 2d */
        cap            = algn_mtx_size_of_2d_matrix (len_char1, len_char2);
        cap_precalcMtx = (1 << alphabetSize) * len_char1;
        cap_dir        = (len_char1 + 1) * (len_char2 + 1);
    } else {                       /* If the size setup is for 3d */
        cap            = algn_mtx_size_of_3d_matrix (len_char1, len_char2, len_char3);
...
void
algn_mtx_print_algn_2d (alignment_matrices_t *alignMtx, size_t w, size_t h) {
...
void
algn_mtx_print_algn_3d (alignment_matrices_t *alignmentMatrices, size_t w, size_t h, size_t d) {
    unsigned int *costs;
