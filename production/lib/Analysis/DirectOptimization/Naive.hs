...
naiveDOThree node1 node2 node3 = 
    let
        (newNode1, newNode3a, _, isLongera) = naiveDOTwo node1 node3
        (newNode2, newNode3b, _, isLongerb) = naiveDOTwo node2 newNode3a
    in (newNode1, newNode2, newNode3b, isLongera || isLongerb)
...
firstAlignRow indelCostValue inSeq rowLength position prevCost
    | position == (rowLength + 1) = (mempty, mempty, mempty)
    | position == 0 = (singleton 0, charToSeq gapChar, singleton DiagDir) <> firstAlignRow indelCostValue inSeq rowLength (position + 1) 0
    | newState /= gapChar = --trace ("new state on first row " ++ show newState) $ -- if there's no indel overlap
        (singleton $ prevCost + indelCostValue, charToSeq newState, singleton LeftDir) <> firstAlignRow indelCostValue inSeq rowLength (position + 1) (prevCost + indelCostValue)
    | otherwise = --trace ("new state on first row, otherwise " ++ show newState) $ -- matching indel so no cost
        (singleton prevCost, charToSeq newState, singleton LeftDir) <> firstAlignRow indelCostValue inSeq rowLength (position + 1) prevCost
        where
...
getOverlapState compChar (Just x) = compChar `op` x
  where
    op = if compChar .&. x == zeroBits then (.|.) else (.&.)

-- | Main recursive function to get alignment rows
getAlignRows :: SeqConstraint s b => s -> s -> Costs -> Int -> AlignRow s -> AlignMatrix s
getAlignRows seq1 seq2 costValues rowNum prevRow
    | rowNum == numChars seq2 + 1 = AlignMatrix (zero 0 0) empty (matrix 0 0 (const LeftDir))
    | otherwise = 
        let thisRow = generateRow seq1 seq2 costValues rowNum prevRow (0, 0)
        in thisRow `joinMat` getAlignRows seq1 seq2 costValues (rowNum + 1) thisRow
...
generateRow seq1 seq2 costvals@(indelCostValue, subCostValue) rowNum prevRow@(costValues, _, _) (position, prevCost) 
    ...
    | position == 0 && newState /= gapChar = (singleton $ upValue + indelCostValue, charToSeq newState, singleton DownDir) <> nextCall (upValue + indelCostValue)
    ...
    | otherwise = --trace "minimal case" $ 
        ...
        where
            ...
            leftCost      = overlapCost char1 indelCostValue + prevCost
            downCost      = overlapCost char2 indelCostValue + upValue
            diagVal       = costValues ! (position - 1)
            intersect     = char1 .&. char2
            union         = char1 .|. char2

            (diagCost, diagState) = if intersect == zeroBits 
                                    then (diagVal + subCostValue, union)
...
traceback alignMat seq1' seq2' = tracebackInternal alignMat seq1' seq2' (numChars seq1', numChars seq2')
    where
        ...
        tracebackInternal alignMatrix seq1 seq2 (row, col) 
            | length (seqs alignMatrix) < row - 1 || nrows (traversal alignMatrix) < row - 1 || ncols (traversal alignMatrix) < col - 1 = error "Traceback cannot function because matrix is incomplete"
            | row == 0 && col == 0 = (emptySeq, emptySeq, emptySeq)
            | curDirect == LeftDir = tracebackInternal alignMatrix seq1 seq2 (row, col - 1) <> (curState, charToSeq gapChar, charToUnMaybe $ seq2 `grabSubChar` (col - 1)) 
            | curDirect == DownDir = tracebackInternal alignMatrix seq1 seq2 (row - 1, col) <> (curState, charToUnMaybe $ seq1 `grabSubChar` (row - 1), charToSeq gapChar) 
            | curDirect == DiagDir = tracebackInternal alignMatrix seq1 seq2 (row - 1, col - 1) <> (curState, charToUnMaybe $ seq1 `grabSubChar` (row - 1), charToUnMaybe $ seq2 `grabSubChar` (col - 1))
            | otherwise = error "Incorrect direction in matrix traversal for alignment"
                where
                    curDirect = getElem row col (traversal alignMatrix)
                    curState = charToUnMaybe $ seqs alignMatrix ! row `grabSubChar` col
