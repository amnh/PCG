...
monoidProperties = testProperty "InsertionEvents is a proper Monoid instance" f
  where
    f :: InsertionEvents Char -> InsertionEvents Char  -> InsertionEvents Char -> Bool
    f x y z = runEqual (==) $ prop e x y z
--    e :: T (InsertionEvents Char)
    e = T
    prop :: Eq a => T (InsertionEvents a) -> InsertionEvents a -> InsertionEvents a -> InsertionEvents a -> Equal (InsertionEvents a)
    prop = prop_Monoid
