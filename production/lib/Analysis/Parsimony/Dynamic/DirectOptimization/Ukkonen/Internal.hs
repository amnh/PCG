...
ukkonenDO char1 char2 costStruct = handleMissingCharacter char1 char2 result
  where
    indelCost = 1
    subCost   = 1
...
ukkonenCore lSeq lLength rSeq rLength maxGap indelCost subCost
  | (gapped ! 0) /= 0 = --trace (show nwMatrix) 
        (cost, filterGaps gapped, gapped, constructDynamic alignLeft, constructDynamic alignRight)
...
--    median       = V.filter (/= gap) medianGap
    gapped = constructDynamic medianGap
...
getRowsUkkonen lSeq rSeq indelCost subCost rowNum prevRow maxGap
  ...
  where 
    ...
    thisRowNonZero = V.cons (barrierCost, gap, DownDir) (getThisRowUkkonen lSeq rSeq indelCost subCost rowNum prevRow startPosition  (V.length lSeq) barrierCost maxGap )
...
getThisRowUkkonen lSeq rSeq indelCost subCost rowNum prevRow position rowLength prevCost maxGap
  ...
  | position == 0 =
      let x | (newState /= gap) =
            ...
            | otherwise = 
                ...
                where
                    newState = getUnionIntersectionState gap (rSeq V.! (rowNum - 1))
  ...
  where
    ...
    ( minCost, minState, minDir) = getMinCostDir leftCost downCost diagCost diagState 
        (getUnionIntersectionState gap (lSeq V.! lSeqPos)) (getUnionIntersectionState gap (rSeq V.! rSeqRow)) 
