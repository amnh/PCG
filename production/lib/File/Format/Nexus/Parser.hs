...
import           Text.Megaparsec.Lexer   (integer, skipBlockCommentNested)
...
parseNexusStream :: FilePath  -- ^ Used in error message reporting
                 ...
                 -> Either (ParseError (Token String) Void) Nexus
...
parseNexus :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m NexusParseResult
parseNexus = nexusFileDefinition


-- |
-- The high level spacification of the Nexus file format.
nexusFileDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m NexusParseResult
nexusFileDefinition = {-do
    x <- getInput
    trace ("nexusFileDefinition"  ++ show x) $ -}do
    _           <- optional (string' (fromString "#NEXUS") <* space)
...
-- should be captured.
ignoredBlockDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m IgnBlock
...
blockend :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m (Tokens s)
blockend = do
  v <- string' $ fromString "end"
  _ <- optional . try . string' $ fromString "block"
  _ <- void $ char ';'
  pure v


nexusBlock :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m NexusBlock
nexusBlock = do
        _      <- symbol . string' $ fromString "BEGIN"
        block' <- symbol block
        _      <- symbol blockend
        pure block'
    where
...
characterBlockDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => String -> Bool -> m PhyloSequence
characterBlockDefinition whichx isAlignedSequence = {-do
    x <- getInput
    trace ("characterBlockDefinition "  ++ show x) $ -}do
    _             <- symbol . string' . fromString  $ whichx <> ";"
    (v,w,x,y,z,a) <- partitionSequenceBlock <$> some seqSubBlock
    pure $ PhyloSequence isAlignedSequence v w x y z a whichx


taxaBlockDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m TaxaSpecification
taxaBlockDefinition = do
    _     <- symbol . string' $ fromString "taxa;"
    (y,z) <- partitionTaxaBlock <$> many taxaSubBlock
    pure $ TaxaSpecification y z


taxaSubBlock :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m SeqSubBlock
taxaSubBlock = {-do
    x <- getInput
...
assumptionsBlockDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m AssumptionBlock
assumptionsBlockDefinition = {-do
    x <- getInput
    trace ("assumptionsBlockDefinition"  ++ show x) $ -}do
        _                <- symbol . string' $ fromString "assumptions;"
...
assumptionFieldDef :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m AssumptionField
...
tcmMatrixDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m StepMatrix
tcmMatrixDefinition = {-do
    x <- getInput
    trace ("\n\ntcmMatrixDefinition "  ++ show x) $ -}do
        _            <- symbol . string' $ fromString "usertype"
        matrixName   <- symbol $ somethingTill spaceChar
        _            <- symbol . optional . choice $ try . string' . fromString <$> ["(stepmatrix)","(realmatrix)"]
        _            <- symbol $ char '='
...
treeBlockDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m TreeBlock
treeBlockDefinition = do
        _     <- symbol . string' $ fromString "trees;"
...
seqSubBlock :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m SeqSubBlock
seqSubBlock = {-do
    x <- getInput
...
dimensionsDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m DimensionsFormat
dimensionsDefinition = {-do
        x         <- getInput
        trace ("**dimensionsDefinition:  " ++ show x) $ -}do
        _         <- symbol   . string' $ fromString "dimensions"
        newTaxa'  <- optional . try . symbol . string' $ fromString "newTaxa"
        _         <- optional . try . symbol . string' $ fromString "nTax"
        _         <- optional . try . symbol $ char '='
        numTaxa'  <- optional . try $ symbol integer
        _         <- optional $ symbol (string' (fromString "nchar") *> optional (char' 's'))
...
formatDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m CharacterFormat
formatDefinition = {-do
        x <- getInput
        trace ("\n\nmany formatDefinition "  ++ show x) $ -}do
        _                         <- symbol . string' $ fromString "format"
...
charFormatFieldDef :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m [CharFormatField]
charFormatFieldDef = {-do
    ...
    where
        block = choice
            [ CharDT      <$> try (string' (fromString "datatype") *> symbol (char '=') *> charDTDefinition)
        ...
        charDTDefinition = choice
                         [ string' (fromString "standard"  ) $> Standard
                         , string' (fromString "dna"       ) $> DNA
                         , string' (fromString "rna"       ) $> RNA
                         , string' (fromString "nucleotide") $> Nucleotide
                         , string' (fromString "protein"   ) $> Protein
                         , string' (fromString "continuous") $> Continuous
                         ] <?> "\nDatatype is not recognized."

-- |
-- Parses one of the two caputured field values present in a tree block.
treeFieldDef :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m TreeField
treeFieldDef = choice
...
booleanDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => String -> m Bool
booleanDefinition blockTitle = {-do
        x <- getInput
        trace (("booleanDefinition " ++ blockTitle)  ++ show x) $-}
    symbol (string' (fromString blockTitle)) $> True

-- | stringDefinition takes a string of format TITLE=value;
-- and returns the value. The semicolon is not captured by this fn.
-- A test exists in the test suite.
stringDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => String -> m String
stringDefinition blockTitle = do
    _     <- symbol . string' $ fromString blockTitle
    _     <- symbol $ char '='
...
quotedStringDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => String -> m (Either String [String])
...
stringListDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => String -> m [String]
stringListDefinition label = {-do
    x <- getInputs
    trace (("many stringListDefinition " ++ label)  ++ show x) $ -}do
    _        <- symbol . string' $ fromString label
...
delimitedStringListDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => String -> Char -> m [String]
...
treeDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m (String, NewickNode)
treeDefinition = do
    _      <- symbol . string' $ fromString "tree"
...
seqMatrixDefinition :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m [String]
seqMatrixDefinition = {-do
    x <- getInput
    trace ("\n\nseqMatrixDefinition "  ++ show x) $ -}do
    _         <- symbol . string' $ fromString "matrix"
...
-- A test exists in the test suite.
ignoredSubBlockDef :: (FoldCase (Tokens s), IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => Char -> m String
...
-- whitespace.
trimmed :: (IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m a -> m a
trimmed x = whitespace *> x <* whitespace


-- |
-- Parses the provided definition and consumes any trailing whitespace.
symbol :: (IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m a -> m a
...
whitespace :: (IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m ()
...
whitespaceNoNewlines :: (IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m ()
...
commentDefinition :: (IsString (Tokens s), MonadParsec e s m, Token s ~ Char {- , Show s -}) => m (Tokens s)
--commentDefinition = comment (string (fromString "[")) (string "]")
commentDefinition = do
    (ts,_) <- match $ skipBlockCommentNested (fromString "[") (fromString "]")
    pure ts
