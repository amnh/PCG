...
void do2D_affine( unsigned int * tcm );
void do3D( unsigned int * tcm );
...
/******************************** set up and allocate all variables and structs ************************************/
    ...
    if (DO_2D)     do2D_nonAffine( tcm );

    if (DO_2D_AFF) do2D_affine( tcm );
...
void do2D_nonAffine( unsigned int * tcm )
{
    cost_matrices_2d_p *costMtx2d_affine = malloc(sizeof(struct cost_matrices_2d_p));
