...
// #define total_poss_align_len 64
...
/****************  Allocate sequences  ****************/
    ...
    int *longest_vals  =malloc( sizeof(int) );
    int *middle_vals   = malloc( sizeof(int) );
    int *shortest_vals =malloc( sizeof(int) );

    size_t longSeqLen   = 5,
           middleSeqLen = 5,
           shortSeqLen  = 5;

    longest_vals  = realloc( NONE, longSeqLen     * sizeof(int) );
    middle_vals   = realloc( NONE, middleSeqLen   * sizeof(int) );
    shortest_vals = realloc( NONE, shortestSeqLen * sizeof(int) );

    size_t total_poss_align_len = longSeqLen + middleSeqLen + shortSeqLen + 3;

    int alphSize = 5; // includes gap, but no ambiguities

    // TODO: find a nice way to do this, since this won't compile, and loops suck.
    longest_vals  = {8, 2, 1, 8, 1}; // don't forget to change lengths!!!
    middle_vals   = {8, 2, 1, 8, 1}; // don't forget to change lengths!!!
    shortest_vals = {8, 2, 1, 8, 1}; // don't forget to change lengths!!!

    dyn_character_t *longSeq   = initializeSeq(total_poss_align_len, longest_vals,  longSeqLen);
    dyn_character_t *shortSeq  = initializeSeq(total_poss_align_len, shortest_vals, shortSeqLen);
    dyn_character_t *mediumSeq = initializeSeq(total_poss_align_len, middle_vals,   middleSeqLen);
    ...
    dyn_character_t *retMediumSeq = initializeSeq(total_poss_align_len, 0, 0);
            ...
            //***** for following seqs, affine requires gap at start of sequence!!! *****/
    printf("length1: %zu, length2: %zu, length3: %zu\n", longSeqLen, shortSeqLen, middleSeqLen);
...
/************************************************ Do 3d alignment *************************************************/
    ...
    // Next this: algn_get_median_3d (dyn_character_t *seq1, dyn_character_t *seq2, dyn_character_t *seq3,
