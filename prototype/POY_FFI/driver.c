...
nw_matrices_p initializeNWMtx() {
    nw_matrices_p newMtx = malloc( sizeof(struct nwMatrices) );
...
seq_p initializeSeq(size_t allocSize, const int *vals, size_t length) {
    ...
    retSeq->seq_begin    = retSeq->array_head + allocSize - retSeq->len; // because the assigned values are at the end of the array
    retSeq->end          = retSeq->seq_begin  + retSeq->len;
...
/************************************************ Do 2d affine alignment *****************************************************/
    ...
    if (DO_AFF) {
       ...
       // }
        ...
        algnCost = algn_fill_plane_3_affine (shortSeq,
                                             longSeq,
                                             shortSeq->len - 1,
                                             longSeq->len - 1,
                                             final_cost_matrix,
