...
type operator =
  ...
  | Op_q
  | Op_Q
  | Op_cm of Transform.transform_matrix
  | Op_m of float * float
  | Op_l of float * float
  ...
  | Op_TD of float * float
  | Op_Tm of Transform.transform_matrix
  | Op_T'
  | Op_Tj of string
  ...
  | Op_k of float * float * float * float
  | Op_sh of string
  | InlineImage of (Pdf.pdfobject * Utility.bytestream)
  | Op_Do of string
  | Op_MP of string
...
dictionary. *)
val parse_stream :
  Pdf.pdfdoc -> Pdf.pdfobject -> Utility.bytestream list -> operator list
...
over several streams), return a list of operators. Should raise PDFError if bad
content. *)
val parse_operators :
  Pdf.pdfdoc -> Pdf.pdfobject -> Pdf.pdfobject list -> operator list

(** Flatten a list of operators to an uncompressed PDF stream. *)
val stream_of_ops : operator list -> Pdf.pdfobject
...
val string_of_ops : operator list -> string
