...
(* $Id: mpi.mli 2 2004-06-22 15:40:20Z andres $ *)
...
val isend: 'a -> rank -> tag -> communicator -> bool
        (* [MPI.isend d dst tag comm] try to send a message containing data [d]
           to the node with rank [dst] in communicator [comm], with tag [tag].
           This function is a non blocking send, so it returns a bool that is 
           true if the operation was successful, otherwise returns flase. *)
...
(* Andres Varon *)
val iprobe: rank -> tag -> communicator -> bool * rank * tag
        (* [Mpi.iprobe source_rank msg_tag comm] is a non blocking instruction
           that checks if a message comming from the process with rank
           [source_rank] in [communicator] with tag [msg_tag]. If such message
           arrived the function is true, else is false. *)
(* Andres Varon *)
...
val intercomm_create: communicator -> rank -> communicator -> rank -> int ->
  communicator
        (* Added by Andres Varon
           [Mpi.intercomm_create local_comm local_leader peer_comm 
           remote_leader tag] creates an intercommunicator between [local_comm]
           and [peer_comm] using [local_leader] and [remote_leader] in the
           communicator creation and the inter-communicators communications. *)
