 ...
 * - Modified by Andres Varon to eliminate sequence escaping
 *)
  ...
  rule token c = parse
    ...
    | '"'
        { with_curr_loc string c;
          let s = buff_contents c in STRING (s, s)             }
    | "'" (newline as x) "'"
        { update_loc c None 1 false 1; CHAR (x.[0], x)               }
    | "'" ( [^ '\\' '\010' '\013']
          | '\\' (['\\' '"' 'n' 't' 'b' 'r' ' ' '\'']
                |['0'-'9'] ['0'-'9'] ['0'-'9']
