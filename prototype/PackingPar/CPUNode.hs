...
allSelect = [fst16, snd16, thd16, fth16] 
...
(.&.) (A16 bit1) (A16 bit2) = 
    let 
        a   | (V.length bit1) /= (V.length bit2) = error "Attempt to take and of bits of different lengths"
            | otherwise = (zipWith (\b1 b2 -> b1 B..&. b2) (V.toList bit1) (V.toList bit2)) `using` parListChunk numChunk rpar
    in A16 $ V.fromList a
(.&.) (S16 bit1) (S16 bit2) = 
    let 
        a   | (V.length bit1) /= (V.length bit2) = error "Attempt to take and of bits of different lengths"
...
complement (A16 bita16) = A16 $ V.fromList (map (\b -> B.complement b) (V.toList bita16) `using`parListChunk numChunk rpar)
complement (S16 bits16) = S16 $ V.fromList (map (\b -> B.complement b) (V.toList bits16) `using`parListChunk numChunk rpar)
complement (A64 bita64) = A64 $ V.fromList (map (\b -> B.complement b) (V.toList bita64) `using`parListChunk numChunk rpar)
complement (S64 bits64) = S64 $ V.fromList (map (\b -> B.complement b) (V.toList bits64) `using`parListChunk numChunk rpar)
complement (SInf biti) = SInf $ B.complement biti
...
shift (A16 bita16) s = A16 $ V.fromList (map (\b -> B.shift b s) (V.toList bita16) `using`parListChunk numChunk rpar)
shift (S16 bits16) s = S16 $ V.fromList (map (\b -> B.shift b s) (V.toList bits16) `using`parListChunk numChunk rpar)
shift (A64 bita64) s = A64 $ V.fromList (map (\b -> B.shift b s) (V.toList bita64) `using`parListChunk numChunk rpar)
shift (S64 bits64) s = S64 $ V.fromList (map (\b -> B.shift b s) (V.toList bits64) `using`parListChunk numChunk rpar)
shift (SInf biti) s = SInf $ B.shift biti s

-- | Rotate function for bits
rotate EmptyPackNode _ = EmptyPackNode
rotate (A16 bita16) r = A16 $ V.fromList (map (\b -> B.rotate b r) (V.toList bita16) `using`parListChunk numChunk rpar)
rotate (S16 bits16) r = S16 $ V.fromList (map (\b -> B.rotate b r) (V.toList bits16) `using`parListChunk numChunk rpar)
rotate (A64 bita64) r = A64 $ V.fromList (map (\b -> B.rotate b r) (V.toList bita64) `using`parListChunk numChunk rpar)
rotate (S64 bits64) r = S64 $ V.fromList (map (\b -> B.rotate b r) (V.toList bits64) `using`parListChunk numChunk rpar)
rotate (SInf biti) r = SInf $ B.rotate biti r
...
popCount (A64 b) = 
    let 
        numbits = V.length  b
        masked16 = map (\mask -> (.|.) (S64 $ V.replicate numbits mask) (A64 b)) allSelect `using`parListChunk numChunk rpar
        shifts = [48, 32, 16, 0]
        maskedRight = zipWith (\s node -> shift node (-s)) shifts masked16 `using`parListChunk numChunk rpar
        convRight = map (\(A64 node) -> V.map (\bs -> fromIntegral bs :: Word16) node) maskedRight `using`parListChunk numChunk rpar
...
blockShiftAndFold sideMode foldMode (A16 inbits) blocklens _ (A16 initVal) = 
    let 
        c   | sideMode == "L" && foldMode == "&" = V.fromList (zipWith3 (\b len iVal -> foldr (\s acc -> acc B..&. (B.shiftR b s)) iVal [1..len-1]) bitList lenList initList `using` parListChunk 6 rpar)
            | sideMode == "R" && foldMode == "&" = V.fromList (zipWith3 (\b len iVal -> foldr (\s acc -> acc B..&. (B.shiftR b s)) iVal [1..len-1]) bitList lenList initList `using` parListChunk 6 rpar)
            | sideMode == "L" && foldMode == "|" = V.fromList (zipWith3 (\b len iVal -> foldr (\s acc -> acc B..|. (B.shiftL b s)) iVal [1..len-1]) bitList lenList initList `using` parListChunk 6 rpar)
            | sideMode == "R" && foldMode == "|" = V.fromList (zipWith3 (\b len iVal -> foldr (\s acc -> acc B..|. (B.shiftR b s)) iVal [1..len-1]) bitList lenList initList `using` parListChunk 6 rpar)
...
getNodeCost node mode blockLens alphLen
    ...
    | bitLen mode == 16 = 
        let 
            (A16 bits) = node
            counts = map (\b -> cardinalityLookup b cardTable) (V.toList bits) `using`parListChunk numChunk rpar
            costs = zipWith (\l c -> div c l) (V.toList blockLens) counts `using`parListChunk numChunk rpar
        in fromIntegral $ sum costs 
    | otherwise =         
        let 
            numbits = div (fromJust $ bitSizeMaybe node) 64
            masked16 = --trace ("masked 16 " ++ show numbits)
                            map (\mask -> (.|.) (A64 $ V.replicate numbits mask) node) allSelect `using`parListChunk numChunk rpar
            shifts = [48, 32, 16, 0]
            maskedRight = zipWith (\s n -> shift n (-s)) shifts masked16 `using`parListChunk numChunk rpar
            convRight = --trace ("convRight " ++ show maskedRight)
                            map (\(A64 n) -> V.map (\b -> fromIntegral b :: Word16) n) maskedRight `using`parListChunk numChunk rpar
            counts = map (\n -> popCount (A16 n)) convRight 
            costs = zipWith (\l c -> div c l) (V.toList blockLens) counts
        in fromIntegral $ sum costs
