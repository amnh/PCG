...
instance Bits BitPackedNode where
    ...
    complement (A16 b) = 
        ...
        in A16 $ V.fromList $ A.toList $ AI.run $ A.map (\bs -> complement bs) (A.use array)
    complement (S16 b) = 
        let
            shape = A.Z A.:. (V.length b)
            array = A.fromList shape (V.toList b) :: A.Vector A.Word16
        in S16 $ V.fromList $ A.toList $ AI.run $ A.map (\bs -> complement bs) (A.use array)
    complement (A64 b) = 
        let
            shape = A.Z A.:. (V.length b)
            array = A.fromList shape (V.toList b) :: A.Vector A.Word64
        in A64 $ V.fromList $ A.toList $ AI.run $ A.map (\bs -> complement bs) (A.use array)
    complement (S64 b) = 
        let
            shape = A.Z A.:. (V.length b)
            array = A.fromList shape (V.toList b) :: A.Vector A.Word64
        in S64 $ V.fromList $ A.toList $ AI.run $ A.map (\bs -> complement bs) (A.use array)
    complement (SInf b) = SInf $ complement b
    ...
    rotate (A16 b) r =
        let
            shape = A.Z A.:. (V.length b)
            array = A.fromList shape (V.toList b) :: A.Vector A.Word16
        in A16 $ V.fromList $ A.toList $ AI.run $ A.map (\bs -> rotate bs r) (A.use array)
    rotate (S16 b) r =
        let
            shape = A.Z A.:. (V.length b)
            array = A.fromList shape (V.toList b) :: A.Vector A.Word16
        in S16 $ V.fromList $ A.toList $ AI.run $ A.map (\bs -> rotate bs r) (A.use array)
    rotate (A64 b) r = 
        let
            shape = A.Z A.:. (V.length b)
            array = A.fromList shape (V.toList b) :: A.Vector A.Word64
        in A64 $ V.fromList $ A.toList $ AI.run $ A.map (\bs -> rotate bs r) (A.use array)
    rotate (S64 b) r = 
        let
            shape = A.Z A.:. (V.length b)
            array = A.fromList shape (V.toList b) :: A.Vector A.Word64
        in S64 $ V.fromList $ A.toList $ AI.run $ A.map (\bs -> rotate bs r) (A.use array)
    rotate (SInf b) r = SInf $ rotate b r
    ...
    testBit (S64  b) index = 
        let 
            ...
            myPos = rem index 64
        in testBit ( b V.! myBit) myPos
    testBit (SInf  b) index = testBit  b index
...
blockShiftAndFold sideMode foldMode (A16 inbits) blocklens _ (A16 initVal) = 
    let 
        c   | sideMode == "L" && foldMode == "&" = V.fromList (zipWith3 (\b len iVal -> foldr (\s acc -> (.&.) acc (shiftR b s)) iVal [1..len-1]) bitList lenList initList `using` parListChunk 6 rpar)
            | sideMode == "R" && foldMode == "&" = V.fromList (zipWith3 (\b len iVal -> foldr (\s acc -> (.&.) acc (shiftR b s)) iVal [1..len-1]) bitList lenList initList `using` parListChunk 6 rpar)
            | sideMode == "L" && foldMode == "|" = V.fromList (zipWith3 (\b len iVal -> foldr (\s acc -> (.|.) acc (shiftL b s)) iVal [1..len-1]) bitList lenList initList `using` parListChunk 6 rpar)
            | sideMode == "R" && foldMode == "|" = V.fromList (zipWith3 (\b len iVal -> foldr (\s acc -> (.|.) acc (shiftR b s)) iVal [1..len-1]) bitList lenList initList `using` parListChunk 6 rpar)
