    ...
    case descendants root of
        ...
        xs -> allSubtrees {- pure $ foldl makeNode [] subtrees -}
            where
                ...
                f (left, right) = -- trace ("leftSubtrees " <> toList leftSubtrees <> "\nrightSubtrees " <> toList rightSubtrees) $
...
generateSubsetPairs previous inNodes originalLength curLength = trace ("inNodes " <> show inNodes) $
    ...
    where
        ...
        getPieces (forestTupleAcc, following) curMember = trace (show curMember) $ (firstSet <> secondSet <> forestTupleAcc, following)
            where
