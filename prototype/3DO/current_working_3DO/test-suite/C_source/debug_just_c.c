...
/** Following is for future, to get sequences at random.

    size_t numSeqsToAlgn = 3,
           i,                         // for use as loop index
           numSeqsInDataset = 17;     // chel has 17 seqs

    int *whichLines = malloc(numSeqsToAlgn * sizeof(int));
    int current = 0,
    last = 0;

    // Now choose three lines that will be retrieved from input file (chel) at random. Note that the same sequence cannot be chosed twice.
    srand((unsigned) time(NULL));
    for (i = 0; i < numSeqsToAlgn; i++) {     // always needing 3 seqs
        while (current == last) current = rand() % numSeqsInDataset;
        whichLines[i] = current * 3 + 2; // Again, this is chel-specific math. Annoying.
        last = current;
    }

    // Now open and read file.
    FILE *infile = fopen("../data/chel.fasta", "r");
    size_t whichLine = 0;
    for (i = 0; i < numSeqsToAlgn; i++) {
        rewind(infile);
        while (whichLine < whichLines[i]) {
            // this isn't right --> fseek(infile, '\n');
            whichLine++;
        }
...
**/
    ...
    size_t longSeqLen   = 151,
           middleSeqLen = 141,
           shortSeqLen  = 140;

    const int SEQ_CAPACITY = longSeqLen * middleSeqLen + shortSeqLen + 3;

    int alphSize = 5; // includes gap, but no ambiguities

    // Fix this once we figure out input/output. Which is beside the point here but something I want to learn for myself.
    // int *longest_vals  = realloc(NULL, sizeof(int) * longSeqLen),
    //     *middle_vals   = realloc(NULL, sizeof(int) * middleSeqLen),
    //     *shortest_vals = realloc(NULL, sizeof(int) * shortSeqLen);

    int longest_vals[151] = {8, 2, 2, 1, 4, 1, 2, 8, 4, 1, 2, 4, 4, 8, 2, 2, 1, 2, 2, 4, 2, 8, 2, 4, 4, 4, 2, 4, 1, 2, 8, 4, 8, 2, 1, 4, 4, 2, 2, 8, 4, 1, 1, 2, 1, 8, 8, 2, 2, 4, 8, 2, 4, 4, 8, 8, 8, 2, 4, 1, 2, 1, 4, 1, 8, 8, 8, 8, 8, 2, 2, 2, 4, 2, 4, 4, 8, 4, 2, 8, 2, 8, 8, 2, 4, 4, 8, 4, 1, 4, 8, 4, 8, 2, 4, 2, 4, 4, 1, 4, 4, 2, 2, 4, 1, 2, 1, 1, 4, 8, 8, 8, 1, 2, 8, 8, 8, 4, 1, 1, 1, 1, 1, 1, 8, 8, 1, 4, 1, 4, 8, 4, 2, 8, 2, 1, 1, 1, 4, 2, 1, 4, 4, 2, 4, 4, 8, 2, 1, 2, 2}; // don't forget to change lengths!!!
    int middle_vals[141] = {8, 2, 8, 1, 4, 1, 2, 8, 4, 4, 2, 4, 4, 8, 2, 2, 4, 2, 8, 8, 2, 2, 4, 4, 2, 4, 4, 8, 8, 1, 2, 8, 4, 2, 2, 8, 4, 4, 2, 2, 8, 1, 1, 1, 2, 1, 8, 2, 8, 4, 2, 2, 4, 4, 8, 8, 8, 8, 2, 2, 2, 8, 2, 4, 4, 8, 4, 2, 2, 2, 8, 8, 4, 1, 8, 8, 4, 1, 4, 8, 4, 8, 2, 8, 8, 4, 4, 4, 8, 4, 4, 2, 2, 4, 4, 2, 1, 2, 4, 8, 8, 8, 1, 2, 8, 8, 8, 4, 1, 1, 1, 1, 1, 1, 8, 8, 1, 4, 1, 4, 8, 4, 2, 8, 2, 1, 1, 1, 4, 2, 1, 4, 2, 4, 2, 1, 1, 2, 4, 2, 2}; // don't forget to change lengths!!!
    int shortest_vals[140] = {8, 2, 2, 1, 4, 1, 2, 8, 4, 8, 2, 4, 4, 8, 2, 2, 4, 2, 1, 2, 2, 4, 4, 1, 4, 4, 8, 8, 1, 2, 8, 4, 4, 2, 1, 4, 4, 1, 2, 2, 4, 4, 1, 2, 4, 8, 2, 8, 1, 4, 2, 2, 4, 4, 2, 2, 8, 2, 2, 2, 8, 2, 4, 8, 8, 4, 2, 8, 2, 8, 8, 2, 1, 2, 2, 4, 4, 4, 8, 4, 8, 2, 8, 8, 4, 4, 4, 8, 4, 8, 2, 2, 4, 4, 2, 1, 2, 4, 8, 8, 8, 1, 2, 8, 8, 8, 4, 1, 1, 1, 1, 1, 1, 8, 8, 1, 4, 1, 4, 8, 4, 2, 8, 2, 1, 1, 1, 4, 2, 1, 4, 2, 4, 1, 2, 2, 2, 4, 2, 2}; // don't forget to change lengths!!!
    ...
    seq_p retLongSeq   = initializeSeq(SEQ_CAPACITY, 0, 0);
    seq_p retShortSeq  = initializeSeq(SEQ_CAPACITY, 0, 0);
    seq_p retMediumSeq = initializeSeq(SEQ_CAPACITY, 0, 0);
