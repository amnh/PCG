...
void * setupCostMtx(int* tcm, int alphSize, int gap_open, int is_2d, dyn_character_t *longSeq) {
...
/************  Allocate cost matrices  **************/
    ...
    if (DO_2D) {
        ...
        algnMat_setup_size (algn_mtxs2d, longSeq->len, longSeq->len, 0, 0, costMtx2d->costMatrixDimension);
        n = costMtx2d->costMatrixDimension;
    }
    if (DO_AFF) {
        costMtx2d_affine = setupCostMtx (tcm, alphSize, GAP_OPEN_COST, 1, longSeq);
        algnMat_setup_size (algn_mtxs2dAffine, longSeq->len, longSeq->len, 0, 0, costMtx2d_affine->costMatrixDimension);
...
/************************************************ Do 2d affine alignment *****************************************************/
    ...
    if (DO_AFF) {
        ...
        initialize_matrices_affine( costMtx2d_affine->gap_open_cost
                                  , shortSeq
                                  , longSeq
                                  , costMtx2d_affine
       ...
       // }
        ...
        backtrace_affine( direction_matrix
                        , longSeq
                        , shortSeq
                        , medianSeq
                        , empty_medianSeq
...
/************************************************ Do 3d alignment *************************************************/
    ...
    if (DO_3D) {
        ...
        // for (elem_t *base = retLongSeq->char_begin; base != retLongSeq->end; base++) {
        //     printf("a: %c\n", *base);
        // }
        // for (elem_t *base = retShortSeq->char_begin; base != retShortSeq->end; base++) {
        //     printf("b: %s\n", base);
        // }
    }
