...
matricest initializeNWMtx() {
    matricest newMtx = malloc( sizeof(struct matrices) );
    ...
    newMtx->matrix      = malloc ( sizeof( int ) );
    newMtx->matrix_d    = malloc ( sizeof( DIRECTION_MATRIX ) );
    ...
    newMtx->cube_d      = malloc ( sizeof( int* ) );  // because they're just pointing to matrix and matrix_d
...
void freeNWMtx(matricest input) {
    free (input->matrix);
    free (input->matrix_d);
    free (input->pointers_3d);
    // free (input->cube);    // don't have to deallocate these two,
    // free (input->cube_d);  // because they're just pointing to matrix and matrix_d
    free (input->precalc);

    free(input);
}

seqt initializeSeq(size_t allocSize, const int *vals, size_t length) {
    seqt retSeq = malloc( sizeof(struct seq) );
...
void freeSeq(seqt toFree) {
    free(toFree->head);
    free(toFree);
}

void resetSeqValues(seqt retSeq) {
    //retSeq->end   = retSeq->begin + retSeq->len;
...
void * setupCostMtx(int* tcm, int alphSize, int gap_open, int is_2d) {
    ...
    cmt retMtx;
    if (is_2d) {
        retMtx = malloc( sizeof(struct cm) );
        ...
        for (SEQT ambElem2 = 1; ambElem2 <= 31; ambElem2++) {
            ...
            if (is_2d) {
                ...
                *median_2 = 0;
                ...
                cm_set_cost_2d   (ambElem1, ambElem2, min_2d,    (cmt) retMtx);
                cm_set_median_2d (ambElem1, ambElem2, median_2d, (cmt) retMtx);
                cm_set_worst     (ambElem1, ambElem2, max_2d,    (cmt) retMtx);
            } else {
                for (SEQT ambElem3 = 1; ambElem3 <= 31; ambElem3++) {
                    ...
                    *median_3 = 0;
                    ...
                    cm_set_cost_3d   (ambElem1, ambElem2, ambElem3, min_3d,    (cm_p) retMtx);
                    cm_set_median_3d (ambElem1, ambElem2, ambElem3, median_3d, (cm_p) retMtx);
                    // no worst in 3d
...
// may return cm or cm, so void *
// no longer setting max, as algorithm to do so is unclear: see note below
void * setupCostMtx(int* tcm, int alphSize, int gap_open, int is_2d, seqt longSeq) {
    ...
    if (is_2d) {
        ...
        retMtx_3d = malloc( sizeof(struct cm_3d) );
        cm_set_val_3d( alphSize,
                               combinations,
                               do_aff,
                               gap_open,
                               all_elements,
        ...
        for (SEQT ambElem2 = 1; ambElem2 <= 31; ambElem2++) {
            for (SEQT ambElem3 = 1; ambElem3 <= 31; ambElem3++) {
                ...
                if (!is_2d) {
                    ...
                    cm_set_cost_3d   (ambElem1, ambElem2, ambElem3, minCost3d, retMtx_3d);
                    cm_set_median_3d (ambElem1, ambElem2, ambElem3, median3d,  retMtx_3d);
                    // cm_set_worst     (ambElem1, ambElem2, max_2d,    (cmt) retMtx);    // no worst in 3d
                    // if( power_2(ambElem1) && power_2(ambElem2) && power_2(ambElem3)) {
            ...
            // printf("median: %2d,   min:   %2d\n", median2d, minCost2d);
            if(is_2d) {
                cm_set_cost   (ambElem1, ambElem2, minCost2d, retMtx);
                cm_set_median (ambElem1, ambElem2, median2d,  retMtx);
    ...
    if (is_2d) {
        ...
        for ( size_t i = 0; i < longSeq->len; i++) {
            ...
            cm_set_prepend (i, cm_calc_cost(retMtx->cost, gap, seqElem, alphSize), retMtx);
            //printf("%2d", cm_get_cost(alphSize - 1, i, retMtx));
            cm_set_tail    (cm_calc_cost(retMtx->cost, seqElem, gap, alphSize), i, retMtx);
        }
        printf("\n");
    }
    if (is_2d) return retMtx;
    else       return retMtx_3d;
}
...
void freeCostMtx(void * input, int is_2d) {
    ...
    if (is_2d) {
        free( ( (cmt) input )->cost);
        free( ( (cmt) input )->median);
        free( ( (cmt) input )->worst);
        free( ( (cmt) input )->prepend_cost);
        free( ( (cmt) input )->tail_cost);
    } else {
        free( ( (cmt) input )->cost);
        free( ( (cmt) input )->median);
    }
...
/****************  Allocate sequences  ****************/
    ...
    int longest_vals[SEQ_CAPACITY]  = {16, 3, 1, 9, 1, 8, 1}; // don't forget to change lengths!!!
    int longSeqLen                  = 7;
    int middle_vals[SEQ_CAPACITY]   = {16, 3, 9, 9, 7, 5, 1}; // don't forget to change lengths!!!
    int middleSeqLen                = 7;
    int shortest_vals[SEQ_CAPACITY] = {16, 7, 9, 9, 4}; // don't forget to change lengths!!!
    int shortSeqLen                 = 5;

    seqt longSeq   = initializeSeq(SEQ_CAPACITY, longest_vals,  longSeqLen);
    seqt shortSeq  = initializeSeq(SEQ_CAPACITY, shortest_vals, shortSeqLen);
    seqt mediumSeq = initializeSeq(SEQ_CAPACITY, middle_vals,   middleSeqLen);

    size_t total_poss_align_len = longSeqLen + shortSeqLen + middleSeqLen;
    seqt retLongSeq   = initializeSeq(total_poss_align_len, 0, 0);
    seqt retShortSeq  = initializeSeq(total_poss_align_len, 0, 0);
    seqt retMediumSeq = initializeSeq(total_poss_align_len, 0, 0);



/****************  Allocate NW matrices  ****************/

    matricest algn_mtxs2d       = initializeNWMtx();
    matricest algn_mtxs2dAffine = initializeNWMtx();
    matricest algn_mtxs3d       = initializeNWMtx();
...
/**************************************************** Do 2d alignment ********************************************************/
    ...
    if (DO_2D) {
        ...
        algnCost = algn_nw( longSeq, shortSeq, costMtx2d, algn_mtxs2d, deltawh );

        // if (DEBUG_MAT) {
        //     printf("\n\nFinal alignment matrix: \n\n");
        //     algn_print_dynmtrx( longSeq, shortSeq, algn_mtxs2d );
        // }


        printf("Original 2d sequences:\n");
        print_seq(longSeq, 1);
        print_seq(shortSeq, 2);

        backtrack_2d ( longSeq
                     , shortSeq
                     , retLongSeq
                     , retShortSeq
                     , algn_mtxs2d
        ...
        print_seq(retLongSeq, 1);
        print_seq(retShortSeq, 2);
        ...
        seqt algnSeq = initializeSeq(SEQ_CAPACITY, algnSeqVals, retLongSeq->len);
        free (algnSeqVals);
...
/************************************************ Do 2d affine alignment *****************************************************/
    ...
    if (DO_AFF) {
        ...
        seqt empty_medianSeq  = malloc( sizeof(struct seq) );
        ...
        seqt medianSeq  = malloc( sizeof(struct seq) );
        ...
        // seqt longerSequence = lenLongSeq > lenShortSeq ? longSeq : shortSeq;
        // seqt shorterSequence = lenLongSeq > lenShortSeq ? shortSeq : longSeq;
       ...
       // }
        ...
        algnCost = algn_fill_plane_3_aff ( shortSeq
                                         , longSeq
                                         , shortSeq->len - 1
                                         , longSeq->len - 1
                                         , final_cost_matrix
        ...
        backtrace_affine (direction_matrix, longSeq, shortSeq, medianSeq, empty_medianSeq,
                          retLongSeq, retShortSeq, costMtx2d_affine);

        printf("\nAligned affine 2d sequences\n");
        if (lenLongSeq > lenShortSeq) {
          print_seq(retShortSeq, 1);
          print_seq(retLongSeq, 2);
          printf("median Seq:\n");
          print_seq(medianSeq, 2);
        } else {
          print_seq(retLongSeq, 1);
          print_seq(retShortSeq, 2);
          printf("median Seq:\n");
...
/************************************************ Do 3d alignment *************************************************/
    ...
    if (DO_3D) {
        ...
        print_seq(longSeq,   1);
        print_seq(mediumSeq, 2);
        print_seq(shortSeq,  3);
        printf("\n");
        ...
        print_seq(retLongSeq,   1);
        print_seq(retMediumSeq, 2);
        print_seq(retShortSeq,  3);
    ...
    // Next this: algn_get_median_3d (seqt seq1, seqt seq2, seqt seq3,
    //                cm_p m, seqt sm)
