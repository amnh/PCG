...
int main() {
    ...
    char seqA_main[5] = "AAAA";
    char seqB_main[5] = "CCCC";   //  Yu_Edit: here 100 is the maximum length of the sequences
    //    char aligned[20];
    //struct align temp;
...
int aligner(char *seq1, char *seq2, int wtInsertDel, int wtSub, struct retType* retAlign){
    ...
    const int INIT_ARR_LENGTH = 11; // because I seem to need to allocate 11 different variable-length, mutable arrays
    const int INIT_LENGTH = strlen(seq1) + strlen(seq2) + 1; // 1 for NULL
    
    printf("Entered aligner\n");
    // printf("test!!!!!!!!!%s\n", seq1);
    
    // EDIT: added this to make array size dynamic (I needed to for testing purposes.). Maybe better with a memset? Dunno.
    char* initArr[INIT_ARR_LENGTH];
    for( int i = 0; i < INIT_ARR_LENGTH; i++ ) {
        initArr[i] = malloc(( sizeof(char) ) * INIT_LENGTH);
        if( initArr[i] == NULL ) {
            return 1;
        }
        for( int j = 0; j < INIT_LENGTH; i++ ) {
            initArr[i][j] = '*';
        }
        initArr[i][INIT_LENGTH-1] = '\0';
    }
    printf("Inited array of variable-length strings.\n");
    // char* initArr1 = malloc(( sizeof(char) ) * INIT_LENGTH);
    // char* initArr2 = malloc(( sizeof(char) ) * INIT_LENGTH);
    // char* initArr3 = malloc(( sizeof(char) ) * INIT_LENGTH);
    // char* initArr4 = malloc(( sizeof(char) ) * INIT_LENGTH);
    // char* initArr5 = malloc(( sizeof(char) ) * INIT_LENGTH);

    // Now, test for allocation. Return 1 if it fails.
    // if( initArr1 == NULL || initArr2 == NULL || initArr3 == NULL ) {
    //     return 1;
    // }
    ...
    //     initArr5[i] = '*';
    // }
    // initArr1[INIT_LENGTH-1] = '\0'; //do I need to do this? My C is a little rusty.
    // initArr2[INIT_LENGTH-1] = '\0';
    // initArr3[INIT_LENGTH-1] = '\0';
    // initArr4[INIT_LENGTH-1] = '\0';
    // initArr5[INIT_LENGTH-1] = '\0';
    // END EDIT;
    ...
    struct align path[3]={ {0, 0, initArr[0], 0, 0, 0, 0, 1},
        {0, 0, initArr[1], 0, 0, 0, 0, 1},
        {0, 0, initArr[2], 0, 0, 0, 0, 1}};
    ...
    struct align pathFirstInfinite = {100000, 100000, initArr[4], 0, 0, 0, 0, 1};
    struct align pathSecondInfinite = {100000, 100000, initArr[5], 0, 0, 0, 0, 2};
  ...
  //  struct align pathSecond[3];
    ...
    char seqB[INIT_LENGTH];
    char seqA[INIT_LENGTH];
...
//    int wtSub=10;        //weigth of substitution
    ...
    struct align pathFirst[3]={
        {wtSub, wtSub + 2 * wtSub * wtSub, initArr[6], 1, 1, 1, 1, 1}, //substitution
        {wtInsertDel, wtInsertDel + 2 * wtInsertDel * wtInsertDel, initArr[7], 1, 1, 1, 0, 1}, //inserstion
        {wtInsertDel, wtInsertDel + 2 * wtInsertDel * wtInsertDel, initArr[8], 1, 1, 0, 1, 1}}; //deletion
    
    
    
    pathFirst[0].partialAlign[0] = seqA[0];
    pathFirst[0].partialAlign[INIT_LENGTH-1] = seqB[0];
    
    pathFirst[1].partialAlign[0] = seqA[0];
    pathFirst[1].partialAlign[INIT_LENGTH-1] = '-';
    
    pathFirst[2].partialAlign[0] = '-';
    pathFirst[2].partialAlign[INIT_LENGTH-1] = seqB[0];
    
    
    struct align pathSecond[3]={
        {wtSub*wtSub, wtSub+2*wtSub*wtSub, initArr[9], 1, 1, 1, 1, 2},//substitution
        {wtInsertDel*wtInsertDel, wtInsertDel+2*wtInsertDel*wtInsertDel, initArr[10], 1, 1, 1, 0, 2},//inserstion
        {wtInsertDel*wtInsertDel, wtInsertDel+2*wtInsertDel*wtInsertDel, initArr[11], 1, 1, 0, 1, 2}};//deletion
    
    
    
    pathSecond[0].partialAlign[0]=seqA[0];
    pathSecond[0].partialAlign[INIT_LENGTH-1]=seqB[0];
    
    pathSecond[1].partialAlign[0]=seqA[0];
    pathSecond[1].partialAlign[INIT_LENGTH-1]='-';
    
    pathSecond[2].partialAlign[0]='-';
    pathSecond[2].partialAlign[INIT_LENGTH-1]=seqB[0];
