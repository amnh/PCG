...
inline U_cell_type *U(int ab, int ac, int d, int s) { 
    return getPtr(&myUAllocInfo, ab, ac, d, s); 
};
inline CPType *CP(int ab, int ac, int d, int s)     { 
...
int furthestReached = 1; // TODO: THis was originally -= ???
...
void printTraceBack(struct seq *ra, struct seq *rb, struct seq *rc)
{
    ...
    revCharArray(resultA,0,ai);
    revCharArray(resultB,0,bi);
    revCharArray(resultC,0,ci);
    ...
    checkAlign(resultA, ai, Astr, Alen);
    checkAlign(resultB, bi, Bstr, Blen);
    checkAlign(resultC, ci, Cstr, Clen);

    assert(alignmentCost(states, resultA, resultB, resultC, ai) == finalCost);
}
