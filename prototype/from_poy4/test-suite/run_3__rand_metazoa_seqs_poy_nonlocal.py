...
def createPOYfileText(poyFile, outputFile_name, version):
    ...
    poyFile.write( 'read("2metazoa_cutdown_poy.fasta")\n' )
    poyFile.write("transform(tcm:(1,1))\n")
    poyFile.write('read("./sample.tre")\n')
    poyFile.write('report("ex2d.txt", diagnosis, trees)\n')
...
def main():
    ...
    poy_file_2d = open( "infile_2d.poy", "w" )
    poy_file_3d = open( "infile_3d.poy", "w" )

    poy_file_2d = createPOYfileText(poy_file_2d, "2doutput.txt", 2)
    poy_file_3d = createPOYfileText(poy_file_3d, "3doutput.txt", 3)
    ...
    for run in range(numRuns):
        fastaFile = open("2metazoa_cutdown_poy.fasta", "w")
        for i in range(3):
        ...
        # non-3d run
        STDERR = open('/private/tmp/stderr', 'w')
        start_2d = time.time()
        subprocess.call(['poy5', 'infile_2d.poy'], stderr=STDERR, stdout=STDERR)
        STDERR.close()
        end_2d = time.time()
        ...
        STDERR = open('/private/tmp/stderr', 'w')
        start_3d = time.time()
        subprocess.call(['poy5', 'infile_3d.poy'], stderr=STDERR, stdout=STDERR)
        STDERR.close()
        end_3d = time.time()

        print "Run number {} time: {}".format(run, (end_3d - start_3d) - (end_2d - start_2d)) # total time the C code ran
