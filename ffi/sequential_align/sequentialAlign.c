           ...
           , retType_t    *retAlign)
{
    ...
    int iMatchFirst[3], iMatchSecond[3];
    ...
    aToB0   = getCost_2d(charA[0], charB[0], tcm, alphSize);
    aToGap0 = getCost_2d(charA[0], GAP,     tcm, alphSize);
    gapToB0 = getCost_2d(GAP,     charB[0], tcm, alphSize);
    ...
    do {
        ...
        aToB0   = getCost_2d( charA[pathFirst[0].input_character_A_ptr], charB[pathFirst[0].input_character_B_ptr], tcm, alphSize );
        aToGap0 = getCost_2d( charA[pathFirst[0].input_character_A_ptr], GAP,                                       tcm, alphSize );
        gapToB0 = getCost_2d( GAP,                                       charB[pathFirst[0].input_character_B_ptr], tcm, alphSize );

        aToB1   = getCost_2d( charA[pathFirst[1].input_character_A_ptr], charB[pathFirst[1].input_character_B_ptr], tcm, alphSize );
        aToGap1 = getCost_2d( charA[pathFirst[1].input_character_A_ptr], GAP,                                       tcm, alphSize );
        gapToB1 = getCost_2d( GAP,                                       charB[pathFirst[1].input_character_B_ptr], tcm, alphSize );

        aToB2   = getCost_2d( charA[pathFirst[2].input_character_A_ptr], charB[pathFirst[2].input_character_B_ptr], tcm, alphSize );
        aToGap2 = getCost_2d( charA[pathFirst[2].input_character_A_ptr], GAP,                                       tcm, alphSize );
        gapToB2 = getCost_2d( GAP,                                       charB[pathFirst[2].input_character_B_ptr], tcm, alphSize );
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (indicatorFirst > 9 && indicatorFirst < 15) { // substitution
                cost = getCost_2d( charA[pathFirst[kFirst].input_character_A_ptr]
            ...
            if ( indicatorFirst > 19 && indicatorFirst < 25) {   // gap in charB
                ...
                pathFirst[i].gapped_partialCost += getCost_2d(charA[pathFirst[kFirst].input_character_A_ptr]
                                                        , GAP
                                                        , tcm
                                                        , alphSize);

                int cost =   getCost_2d(charA[pathFirst[kFirst].input_character_A_ptr]
                                    , GAP
                                    , tcm
                                    , alphSize );
        ...
        if (flag == 0) {
            ...
            for (i = 0; i < 3; i++){
                ...
                if (indicatorSecond > 9 && indicatorSecond < 15) { // substitution
                    cost = getCost_2d( charA[pathFirst[kSecond].input_character_A_ptr]
    ...
    for (i = 0; i < SEQ_MAX_LEN && finalAlign.partialAlign_A[i] && finalAlign.partialAlign_B[i]; i++) {
      //        printf("n+6th a: %2" PRIu64 " b: %2" PRIu64 " \n", finalAlign.partialAlign_A[i], finalAlign.partialAlign_B[i + LENGTH]);
        ...
        if (finalAlign.partialAlign_A[i] == GAP || finalAlign.partialAlign_B[i] == GAP) {
            finalAlign.gapped_partialCost += getCost_2d(GAP, GAP, tcm, alphSize);

        } else if (finalAlign.partialAlign_A[i] == finalAlign.partialAlign_B[i]) {
            finalAlign.gapped_partialCost += getCost_2d(finalAlign.partialAlign_A[i], finalAlign.partialAlign_B[i], tcm, alphSize);
...
int currentAlignmentCost(alignment_t *path, costMatrix_p tcm, size_t maxLen, size_t alphSize)
{
    ...
    for(i = 0; i < path->aligned_character_A_end_ptr && i < maxLen; i++) {
        ...
        cost = getCost_2d(path->partialAlign_A[i], path->partialAlign_B[i], tcm, alphSize);
        costTempFirst  += cost;
