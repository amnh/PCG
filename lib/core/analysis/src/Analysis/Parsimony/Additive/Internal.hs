...
additivePreorder
  ...
  => PreorderBinaryContext d (Word, d')
  -> d'
additivePreorder = \case
  RootContext root -> extendRangedToPreorder root $ root ^. preliminaryInterval

  PreInternalContext (_, parentDecoration) (either id id -> childDecoration)
    | childDecoration ^. isLeaf -> finalizeLeaf childDecoration

  PreInternalContext (_, parentDecoration) (either id id -> childDecoration)
    -> extendRangedToPreorder childDecoration $ determineFinalState childDecoration parentDecoration--}
